    ...
    <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-slate-200/80 shadow-xs pt-[max(0.75rem,env(safe-area-inset-top))] sm:pt-2.5 pb-2.5">
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
          ...
          {/* Unified Account Profile Dropdown */}
          <div className="relative shrink-0">
            <button
              onClick={() => setIsAccountMenuOpen(!isAccountMenuOpen)}
              className="flex items-center gap-1.5 px-2.5 py-1.5 sm:px-3 sm:py-2 rounded-xl bg-slate-100 hover:bg-slate-200/80 border border-slate-200 text-slate-800 font-semibold text-xs transition-all cursor-pointer shrink-0 shadow-xs"
            >
              <div className="w-6 h-6 rounded-full bg-emerald-600 text-white font-bold text-xs flex items-center justify-center shrink-0 shadow-xs">
                {currentUser.name.charAt(0)}
              </div>
              <span className="truncate max-w-[80px] sm:max-w-[130px] font-bold text-slate-900 text-xs sm:text-sm">
            ...
            {isAccountMenuOpen && (
              <>
                ...
                ></div>
                <div className="fixed top-14 right-3 sm:absolute sm:right-0 sm:top-full sm:mt-2 w-[calc(100vw-24px)] max-w-xs sm:w-72 bg-white rounded-2xl shadow-2xl border border-slate-200 p-4 z-50 animate-fade-in space-y-3.5 max-h-[85vh] overflow-y-auto">

                  {/* Account Profile Header */}
                  <div className="p-3 bg-slate-50 rounded-xl border border-slate-100 flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-emerald-700 text-white font-bold text-sm flex items-center justify-center shrink-0 shadow-xs">
                      {currentUser.name.charAt(0)}
                    </div>
